...
function Test-KnownRuntimeCacheEntry {
  ...
  if ($Entry.PSIsContainer) {
    return ($Entry.Name -match '^(mpv|ffmpeg)-extracted-[0-9a-f]{64}  param([Parameter(Mandatory = $true)][string]$Value)
...
foreach ($path in $required) {
  ...
  return ($Entry.Name -match '^(mpv|ffmpeg)-[0-9a-f]{64}\.zip(?:\.download)?  param([Parameter(Mandatory = $true)][string]$Value)
