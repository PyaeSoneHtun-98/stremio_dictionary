...
export interface StremioHandoffInspection {
  ...
  automaticRepairBlocked: boolean
...
export class StremioHandoffService {
  ...
  inspectStatus(): StremioHandoffInspection {
    ...
    for (const path of paths) {
      if (!existsSync(path)) {
        // A recorded target that disappeared is stale state. The disable/reset helper can
        // safely forget it while cleaning the rest of the recorded integration.
        repairNeeded = true
        continue
      }
      ...
      if (!hasAnyMarker) {
        // If a recorded server.js still exists but no longer contains our patch, a Stremio
        // update may have replaced the active file. Do not let another old verified target
        // make the overall integration look healthy.
        repairNeeded = true
        continue
      }
      ...
      if (!configuredExecutable) {
        // The markers exist, but the block is no longer the exact structure generated by
        // Subtitle Bridge. Automated cleanup must fail closed rather than deleting arbitrary
        // user/Stremio code between familiar-looking markers.
        automaticRepairBlocked = true
        continue
      }

      const currentExecutable = sameWindowsPath(configuredExecutable, this.executablePath)
      const compatibleLayout = hasCompatibleStremioLayout(baseText)

      if (currentExecutable && compatibleLayout) {
        verifiedTargets += 1
      } else {
        // Exact generated blocks are safe for the disable helper to remove even if Stremio's
        // surrounding discovery layout changed or the app executable moved.
        repairNeeded = true
      }
    }

    return {
      enabled: verifiedTargets > 0 && !repairNeeded && !automaticRepairBlocked,
      repairNeeded,
